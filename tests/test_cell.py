...
def test_render_range(
    cell_style: PlotCellStyle, num_values: int, expected_values: List[str]
):
...
@pytest.mark.parametrize("cell_style, expected", [(s, s.over) for s in PlotCellStyle])
def test_render_over(cell_style: PlotCellStyle, expected: str):
    value = 8
    value_range = (1, 7)
    result = PlotCellRenderer.render(
        value=value, value_range=value_range, cell_style=cell_style
...
@pytest.mark.parametrize("cell_style, expected", [(s, s.under) for s in PlotCellStyle])
def test_render_under(cell_style: PlotCellStyle, expected: str):
    value = 0
    value_range = (1, 7)
    result = PlotCellRenderer.render(
        value=value, value_range=value_range, cell_style=cell_style
